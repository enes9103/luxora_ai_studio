...
export const getApiUsage = (): ApiUsage => {
  try {
    ...
    if (storedUsage) {
      ...
      return {
        defaultKey:
          typeof parsed.defaultKey === "number" ? parsed.defaultKey : 0,
...
const incrementApiUsage = (isUserKey: boolean) => {
  ...
  const newUsage = {
    ...currentUsage,
    [isUserKey ? "userKey" : "defaultKey"]:
      currentUsage[isUserKey ? "userKey" : "defaultKey"] + 1,
  };
  try {
...
const withRetry = async <T>(apiCall: () => Promise<T>): Promise<T> => {
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      lastError = e;
      if (isTransientError(e) && i < MAX_RETRIES - 1) {
        // Only wait if it's not the last attempt
        const backoffTime =
          INITIAL_BACKOFF_MS * Math.pow(2, i) + Math.random() * 1000; // Add jitter
        console.warn(
          `Transient error detected. Retrying in ${Math.round(
            backoffTime
          )}ms... (Attempt ${i + 1}/${MAX_RETRIES})`,
          e
        );
...
const subjectDetailsSchema = {
  ...
  properties: {
    costume: {
      type: Type.STRING,
      description: "Description of the subject's clothing and accessories.",
    },
    subject_expression: {
      type: Type.STRING,
      description: "A specific facial expression for the subject.",
    },
    subject_action: {
      type: Type.STRING,
      description: "A specific, subtle action the subject is performing.",
    },
  },
...
const promptSchema = {
  ...
  properties: {
    prompt: {
      type: Type.STRING,
      description:
        "A main instruction string summarizing the concept for the image generation model.",
    },
    details: {
      type: Type.OBJECT,
      properties: {
        year: {
          type: Type.STRING,
          description:
            "A specific year or era (e.g., 1985, Cyberpunk Future, Ancient Rome).",
        },
        genre: {
          type: Type.STRING,
          description: "A specific genre, style, or artistic medium.",
        },
        location: {
          type: Type.STRING,
          description: "A vivid description of the setting.",
        },
        lighting: {
          type: Type.ARRAY,
          items: { type: Type.STRING },
          description: "A description of the lighting, can be multiple values.",
        },
        camera_angle: {
          type: Type.STRING,
          description: "A specific camera angle.",
        },
        emotion: {
          type: Type.ARRAY,
          items: { type: Type.STRING },
          description: "The dominant emotion(s) of the scene.",
        },
        color_palette: {
          type: Type.ARRAY,
          items: { type: Type.STRING },
          description: "The key color(s) of the scene.",
        },
        atmosphere: {
          type: Type.ARRAY,
          items: { type: Type.STRING },
          description:
            "A rich, evocative description of the overall mood and atmosphere.",
        },
        environmental_elements: {
          type: Type.STRING,
          description: "Key dynamic or static elements in the environment.",
        },
...
const promptExamples = [
  {
    ...
    details: {
      ...
      location:
        "A narrow, rain-soaked alley between massive skyscrapers with glowing neon signs.",
  ...
  },
  {
    ...
    details: {
      ...
      location:
        "A rocky, mist-covered beach with a dense pine forest and mountains in the background.",
      lighting: [
        "Overcast morning light",
        "Soft, diffused light through the mist",
      ],
      camera_angle:
        "Low-angle wide shot, making the landscape and subjects feel epic.",
      ...
      environmental_elements:
        "Thick fog rolling in from the sea, waves crashing on the shore, a raven perched on a nearby rock.",
      subject1: {
        costume: "Heavy furs, leather armor, and a horned helmet.",
        subject_expression:
          "A determined, weather-beaten face, looking towards the mountains.",
        subject_action: "Leaning on a large, carved battle axe.",
      },
      subject2: {
        costume: "Chainmail over a tunic, with a thick cloak.",
        subject_expression:
          "A curious and awestruck look, taking in the new world.",
        subject_action:
          "Holding a coiled rope over his shoulder, ready for the expedition.",
      },
      negative_prompt: {
        exclude_visuals: [
          "sunshine",
...
const getPromptGenerationContent = (
  ...
  if (cameraAngle && cameraAngle !== "None") {
    promptParts.push(
      `CRITICAL REQUIREMENT: The 'camera_angle' field in the output JSON MUST be exactly '${cameraAngle}'.`
    );
  }
  ...
  const hasDetails = (details?: Partial<SubjectSpecificDetails>): boolean => {
    ...
    return (
      !!details.costume ||
      !!details.subject_action ||
      !!details.subject_expression
    );
  };

  if (hasDetails(subject1Details)) {
    const providedDetails: string[] = [];
    if (subject1Details?.costume)
      providedDetails.push(
        `costume must be exactly: '${subject1Details.costume}'`
      );
    if (subject1Details?.subject_expression)
      providedDetails.push(
        `expression must be exactly: '${subject1Details.subject_expression}'`
      );
    if (subject1Details?.subject_action)
      providedDetails.push(
        `action must be exactly: '${subject1Details.subject_action}'`
      );
    promptParts.push(
      `CRITICAL REQUIREMENT for Subject 1: You MUST use these specific details: ${providedDetails.join(
        "; "
      )}. Fill in any missing details creatively but these are MANDATORY.`
    );
  }

  if (gender2 && hasDetails(subject2Details)) {
    const providedDetails: string[] = [];
    if (subject2Details?.costume)
      providedDetails.push(
        `costume must be exactly: '${subject2Details.costume}'`
      );
    if (subject2Details?.subject_expression)
      providedDetails.push(
        `expression must be exactly: '${subject2Details.subject_expression}'`
      );
    if (subject2Details?.subject_action)
      providedDetails.push(
        `action must be exactly: '${subject2Details.subject_action}'`
      );
    promptParts.push(
      `CRITICAL REQUIREMENT for Subject 2: You MUST use these specific details: ${providedDetails.join(
        "; "
      )}. Fill in any missing details creatively but these are MANDATORY.`
    );
  }
  ...
  if (negativePromptOverrides) {
    const { exclude_visuals, exclude_styles, exclude_colors, exclude_objects } =
      negativePromptOverrides;
    const requirements: string[] = [];
    if (exclude_visuals && exclude_visuals.length > 0)
      requirements.push(`visuals: [${exclude_visuals.join(", ")}]`);
    if (exclude_styles && exclude_styles.length > 0)
      requirements.push(`styles: [${exclude_styles.join(", ")}]`);
    if (exclude_colors && exclude_colors.length > 0)
      requirements.push(`colors: [${exclude_colors.join(", ")}]`);
    if (exclude_objects && exclude_objects.length > 0)
      requirements.push(`objects: [${exclude_objects.join(", ")}]`);

    if (requirements.length > 0) {
      promptParts.push(
        `CRITICAL REQUIREMENT: The generated 'negative_prompt' MUST strictly include these specific exclusions: ${requirements.join(
          "; "
        )}. You MUST incorporate these values into the respective arrays in the final JSON output. You may add other relevant exclusions as well.`
      );
    }
  }

  promptParts.push(
    `The concept you generate MUST be suitable for a ${
      quality === "high" ? "high-quality, highly detailed" : "standard quality"
    } image. CRITICAL: The composition must strictly adhere to a cinematic ${aspectRatio} aspect ratio.`
  );

  const jsonStructure = {
    prompt:
      "A main instruction string that starts with 'You will perform an image edit...' followed by a summary of the concept.",
    details: {
      ...
      subject2: {
        // This key should ONLY be included if the prompt is for two subjects.
  ...
  promptParts.push(
    "CRITICAL: The 'negative_prompt' object MUST be generated for EVERY concept. Tailor it to the specific concept. This is not optional."
  );
  if (gender2) {
    promptParts.push(
      "CRITICAL: Because two subjects were requested, the final JSON object MUST include the `subject2` details object."
    );
  } else {
    promptParts.push(
      "CRITICAL: Because only one subject was requested, the final JSON object must NOT include the `subject2` key."
    );
  }
...
export const generateEditPrompt = async (
  ...
  const response: GenerateContentResponse = await withRetry(() =>
    ai.models.generateContent({
      model: textModel,
      contents: getPromptGenerationContent(
        gender1,
        gender2,
        quality,
        aspectRatio,
      ...
      config: {
        systemInstruction:
          "You are a creative director specializing in generating imaginative and cinematic photo edit concepts. You must output your response as a valid JSON object.",
  ...
  try {
    ...
    if (
      !parsedJson ||
      typeof parsedJson.prompt !== "string" ||
      typeof parsedJson.details !== "object" ||
      !parsedJson.details.subject1
    ) {
      console.error("Parsed JSON is missing required fields:", parsedJson);
      throw new Error(
        "The AI returned a JSON object with an unexpected structure."
      );
    }
...
export const editImageWithGemini = async (
  ...
  if (removeBackground) {
    instructionParts.push(
      "First, perfectly remove the background from all subjects in the image, making it transparent. The subjects must be fully and cleanly isolated."
    );
  }
  ...
  if (imageData2) {
    instructionParts.push(
      "ABSOLUTELY CRITICAL: This prompt involves two people. Use the person from the FIRST uploaded image as Subject 1, and the person from the SECOND uploaded image as Subject 2."
    );
  ...
  if (negative_prompt) {
    ...
    if (allExclusions.length > 0) {
      instructionParts.push(
        `--- NEGATIVE PROMPT: Strictly avoid the following elements and styles: ${allExclusions.join(
          ", "
        )}.`
      );
    }
  ...
  if (quality === "high")
    finalPrompt +=
      " The final image should be of high quality, with fine details and a professional finish.";
