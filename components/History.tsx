...
const HistoryItem: React.FC<HistoryItemProps> = ({ item, onSelect }) => {
  ...
  return (
    <div className="border border-border/70 rounded-xl bg-card/90 transition-all duration-300 shadow-[0_18px_50px_-35px_rgba(0,0,0,0.6)]">
      <button
      ...
      <div className={`transition-[max-height,padding] duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-[40rem]' : 'max-h-0'}`}>
        <div className="p-4 pt-0">
          <div className="border-t border-border/60 pt-4 text-sm space-y-4">
...
const SkeletonHistoryItem: React.FC = () => (
    <div className="border border-border/70 rounded-xl p-4 bg-card/90 animate-pulse flex items-center gap-4 shadow-[0_18px_50px_-35px_rgba(0,0,0,0.6)]">
...
export const History: React.FC<{ prompts: HistoryEntry[], onSelectPrompt: (prompt: Prompt) => void, isGeneratingNewPrompt?: boolean }> = ({ prompts, onSelectPrompt, isGeneratingNewPrompt = false }) => {
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-card/90 shadow-[0_20px_60px_-35px_rgba(0,0,0,0.65)]">
       <div className="p-4 sm:p-6 flex flex-wrap justify-between items-center gap-4 border-b border-border/60">
