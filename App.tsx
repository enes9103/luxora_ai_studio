...
const KeyIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><circle cx="7.5" cy="15.5" r="5.5"/><path d="m21 2-9.6 9.6"/><path d="m15.5 7.5 3 3L22 7l-3-3"/></svg>
);

const ProUpsellModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    if (!isOpen) return null;
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
            <div className="relative w-full max-w-lg rounded-2xl border border-primary/40 bg-card/95 shadow-[0_30px_120px_-50px_rgba(0,0,0,0.9)] overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-primary/15 via-transparent to-transparent pointer-events-none" />
                <div className="relative p-6 sm:p-8 space-y-6">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Go Pro</p>
                            <h3 className="text-2xl font-bold text-card-foreground mt-1">Unlimited renders & priority queue</h3>
                            <p className="text-sm text-muted-foreground mt-2">You’ve used your first 3 credits. Upgrade to Luxora Pro for unlimited tries, priority processing, and high-res outputs.</p>
                        </div>
                    ...
                    <div className="rounded-xl border border-border/70 bg-background/80 p-4 space-y-4">
                        <div className="flex items-baseline justify-between">
                            <div>
                                <p className="text-sm font-semibold text-muted-foreground">Monthly Plan</p>
                                <p className="text-3xl font-bold text-card-foreground">₺299<span className="text-sm text-muted-foreground font-semibold">/mo</span></p>
                            </div>
                            <span className="px-3 py-1 rounded-full bg-primary/15 text-primary text-xs font-semibold border border-primary/25">Most popular</span>
                        </div>
                        <ul className="space-y-2 text-sm text-card-foreground">
                            <li className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-primary/80" /> Unlimited renders + priority queue</li>
                            <li className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-primary/80" /> 4K exports and background removal</li>
                            <li className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-primary/80" /> Style profiles and negative prompt library</li>
                        </ul>
                        <div className="flex flex-col sm:flex-row gap-3">
                            <button className="flex-1 h-11 rounded-md bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition">Upgrade to Pro</button>
                            <button onClick={onClose} className="h-11 px-4 rounded-md border border-border/70 bg-card/70 text-card-foreground hover:bg-muted transition">Later</button>
                        </div>
                    </div>
                </div>
            </div>
...
const Header: React.FC<{ theme: Theme; setTheme: (theme: Theme) => void; onOpenSettings: () => void; isUserKeyActive: boolean; }> = ({ theme, setTheme, onOpenSettings, isUserKeyActive }) => {
    ...
    return (
      <header className="py-4 px-4 sm:px-6 sticky top-0 z-20 border-b border-border/60 bg-background/80 backdrop-blur-xl">
        <div className="container mx-auto flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full border border-border/80 bg-card/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">
              <span className="h-2 w-2 rounded-full bg-primary/80 animate-pulse"></span>
              Luxora AI Studio
            </div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl sm:text-3xl font-bold text-card-foreground">Luxora Visual Lab</h1>
              <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-primary/15 text-primary border border-primary/25">Gemini</span>
            </div>
            <p className="text-sm text-muted-foreground max-w-2xl">Transform portraits into cinematic or experimental looks in minutes. Fine-tune models, camera angles, and negative prompts for precise control.</p>
          </div>
...
const App: React.FC = () => {
  ...
  const [creditsUsed, setCreditsUsed] = useState<number>(() => {
    const stored = localStorage.getItem('creditsUsed');
  ...
  }, [originalImage1, originalImage2, addLog, userApiKey, setCreditsUsed, setIsProModalOpen]);
  ...
  const remainingCredits = Math.max(0, 3 - creditsUsed);
  ...
  return (
    <DebugContext.Provider value={{ logs, addLog }}>
      <div className="flex flex-col min-h-screen">
        ...
        <main className="container mx-auto px-4 sm:px-6 pb-10 flex-grow w-full">
          <div className="space-y-6 lg:space-y-8">
            <div className="relative overflow-hidden rounded-2xl border border-border/70 bg-gradient-to-r from-primary/12 via-card/95 to-card/90 shadow-[0_24px_70px_-40px_rgba(0,0,0,0.65)] p-6 sm:p-8">
              <div className="absolute inset-0 pointer-events-none opacity-40 bg-[radial-gradient(circle_at_20%_20%,rgba(255,255,255,0.14),transparent_30%),radial-gradient(circle_at_80%_0%,rgba(255,255,255,0.08),transparent_25%)]"></div>
              <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-primary uppercase tracking-[0.12em]">Pro Flow</p>
                  <h2 className="text-2xl sm:text-3xl font-bold text-card-foreground">Upload, tailor, and create in one click</h2>
                  <p className="text-muted-foreground max-w-3xl">Upload a shot, set the scene details, and generate with Gemini in minutes. Negative prompts, camera angles, and background removal keep you in control.</p>
                  <div className="flex flex-wrap items-center gap-3 text-sm">
                    <span className="px-3 py-1 rounded-full bg-primary/15 text-primary font-semibold border border-primary/30">Free credits: {remainingCredits > 0 ? `${remainingCredits} left` : 'none left'}</span>
                    <span className="text-muted-foreground">Pro upgrade prompt appears after 3 credits.</span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 min-w-[240px]">
                  <div className="rounded-xl border border-border/60 bg-background/80 px-3 py-3 text-center shadow-sm">
                    <p className="text-xs text-muted-foreground">Metin Modeli</p>
                    <p className="text-sm font-semibold text-card-foreground truncate">{textModel}</p>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-background/80 px-3 py-3 text-center shadow-sm">
                    <p className="text-xs text-muted-foreground">Image Model</p>
                    <p className="text-sm font-semibold text-card-foreground truncate">{imageModel}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      />
      <ProUpsellModal isOpen={isProModalOpen} onClose={() => setIsProModalOpen(false)} />
